// Toast.jsx - Toasted alerts notifications list and items
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 pt-0.5 pr-2">
        <p className="text-sm font-semibold text-brand-slate-900 dark:text-white font-display">
          {type.charAt(0).toUpperCase() + type.slice(1)}
        </p>
        <p className="mt-1 text-xs text-brand-slate-500 dark:text-brand-slate-400 leading-normal">
          {message}
        </p>
      </div>
      <div className="flex-shrink-0 flex">
        <button
          onClick={() => onClose(id)}
          type="button"
          className="inline-flex rounded-md text-brand-slate-400 dark:text-brand-slate-500 hover:text-brand-slate-600 dark:hover:text-brand-slate-350 focus:outline-none"
        >
...
export const ToastContainer = () => {
  ...
  return (
    <div 
      aria-live="assertive" 
      className="fixed bottom-5 right-5 z-55 flex flex-col space-y-3 w-full max-w-sm pointer-events-none"
    >
      {toasts.map(toast => (
        <ToastItem
          key={toast.id}
